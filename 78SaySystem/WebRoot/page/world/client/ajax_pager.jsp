    ...
    <ul>
        <c:choose>
    		...
    		<li>
    			...
    			<a href="${ctx}/world/index/category/${categoryId}/${pageBean.currentPage - 1 }.html">上一页</a>
    			</c:when>
    			<c:otherwise>
    			<a href="${ctx}/world/index/${pageBean.currentPage - 1 }.html">上一页</a>
    			</c:otherwise>
    			</c:choose>
    		</li>
    		</c:when>
      	...
      	<c:forEach items="${pageBean.getShowPage()}" var="sp" varStatus="stat">
      		<c:choose>
      			...
      			<li>
      				<c:choose>
	    			<c:when test="${!empty(categoryId)}">
	    			<a href="${ctx}/world/index/category/${categoryId}/${sp}.html">${sp}</a>
	    			</c:when>
	    			<c:otherwise>
	    			<a href="${ctx}/world/index/${sp}.html">${sp}</a>
	    			</c:otherwise>
	    			</c:choose>
      			</li>
      			</c:otherwise>
        ...
        <c:choose>
          	...
          	<li>
          		<c:choose>
    			<c:when test="${!empty(categoryId)}">
    			<a href="${ctx}/world/index/category/${categoryId}/${pageBean.currentPage + 1}.html">下一页</a>
    			</c:when>
    			<c:otherwise>
    			<a href="${ctx}/world/index/${pageBean.currentPage + 1}.html">下一页</a>
    			</c:otherwise>
    			</c:choose>
         	</li>
          	</c:when>
